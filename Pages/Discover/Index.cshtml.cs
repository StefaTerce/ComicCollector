    ...
    public class IndexModel : PageModel
    {
        ...
        private async Task SearchComicVineAsync(bool isFeaturedSearch)
        {
            try
            {
                var query = new ComicVineSearchQuery
                {
                    ...
                    // FieldList = "id,name,image,description,issue_number,volume,person_credits,cover_date,store_date" // Gi� nel modello
                };
        ...
        private async Task SearchMangaDexAsync(bool isFeaturedSearch)
        {
            try
            {
                ...
                // Questo pu� essere intensivo se ci sono molti risultati.
                foreach (var manga in results)
                {
                    if (string.IsNullOrEmpty(manga.CoverImageUrl)) // Prova a caricare solo se non gi� presente
                    {
                        // Il MangaDexService ora dovrebbe tentare di ottenere il coverId durante SearchMangaAsync
                        // e GetCoverUrlAsync lo user�.
                        // Per i featured, vogliamo assicurarci che le copertine siano caricate.
                        var mangaRelationships = await FetchMangaRelationshipsAsync(manga.Id); // Questo � ancora un placeholder
        ...
        {
            // Se 'forceApiCall' � true, o se le relazioni non sono state ottenute tramite 'includes',
            // si dovrebbe fare una chiamata a /manga/{mangaId}?includes[]=cover_art,author,artist
            // Per ora, questo metodo rimane un placeholder perch� la logica di recupero copertina
            // � principalmente in MangaDexService.GetCoverUrlAsync.
            // Il flag 'forceApiCall' � per un'eventuale implementazione futura pi� granulare.
